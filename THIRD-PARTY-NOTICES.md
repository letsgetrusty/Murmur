...
ISC, Zlib, MPL-2.0, Unicode-3.0, and similar). **No copyleft (GPL/LGPL/AGPL) code
is statically linked into the binary.** (Kokoro's optional espeak-ng grapheme-to-
phoneme backend — which is GPL-3.0 — is deliberately disabled; Kokoro uses a
dictionary-based G2P instead. See `Cargo.toml`.)

---

## Key bundled components

| Component | Via crate | License |
|---|---|---|
| [Tauri](https://tauri.app) (app framework) | `tauri`, plugins | MIT OR Apache-2.0 |
| [whisper.cpp](https://github.com/ggerganov/whisper.cpp) (speech-to-text) | `whisper-rs` / `whisper-rs-sys` | MIT |
| [llama.cpp](https://github.com/ggerganov/llama.cpp) (refine LLM runtime) | `llama-cpp-2` / `llama-cpp-sys-2` | MIT |
| [Kokoro](https://github.com/thewh1teagle/kokoro-onnx) (neural TTS) | `kokoro-en`, `misaki-rs` | MIT / Apache-2.0 |
| [ONNX Runtime](https://onnxruntime.ai) (Kokoro inference) | `ort` / `ort-sys` | MIT OR Apache-2.0 |
| CMU Pronouncing Dictionary (Kokoro G2P) | `cmudict-fast` | BSD-2-Clause |
| [cpal](https://github.com/RustAudio/cpal) (audio capture) | `cpal` | Apache-2.0 |
| [SQLite](https://sqlite.org) (history) | `rusqlite` (bundled) | Public Domain (SQLite) / MIT (rusqlite) |
| macOS FFI | `objc2*` | MIT OR Apache-2.0 |
| enigo, arboard (inject / clipboard) | `enigo`, `arboard` | MIT / Apache-2.0 |

### MPL-2.0 components

A few transitive dependencies of the webview stack are under the Mozilla Public
License 2.0 (`cssparser`, `cssparser-macros`, `dtoa-short`, `selectors`,
`option-ext`). MPL-2.0 is file-level copyleft: it permits use in a larger work
under other terms, and only requires sharing modifications *to those files*. We
do not modify them; upstream source is available from crates.io.
...
|---|---|---|---|
| Whisper (`ggml-*.bin`) | Speech-to-text | MIT | [ggerganov/whisper.cpp](https://huggingface.co/ggerganov/whisper.cpp) |
| Qwen3-1.7B (GGUF) | Refinement LLM | Apache-2.0 | [Qwen](https://huggingface.co/Qwen) |
| Kokoro v1.0 (ONNX) + voices | Neural TTS | Apache-2.0 | [hexgrad/Kokoro-82M](https://huggingface.co/hexgrad/Kokoro-82M) |
...
- cmudict-fast 0.8.0 — MIT/Apache-2.0
...
- kokoro-en 0.1.5 — Apache-2.0
- language-tokenizer 0.1.0 — WTFPL (see source)
...
- num2words 0.4.1 — MIT OR Apache-2.0
...
- waken_snowball 0.1.0 — BSD-3-Clause
